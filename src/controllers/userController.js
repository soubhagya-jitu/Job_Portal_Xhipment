...
const createUser = async (req, res) => {
    try {
        ...
        if (!validation.isValidName(firstName)) return res.status(400).send({ status: false, message: "first name is not valid" })
        ...
        res.status(400).send({ status: false, data:createUser,message: "Successfully created" })
...
const loginUser = async function (req, res) {
    try {
      ...
      if (!bcryptPass) return res.status(400).send({ status: false, message: "The entered password is wrong" })
